...
const requireCatalog = process.env.SHOPIFY_REQUIRE_LIVE_CATALOG === "true";
...
describe.skipIf(!configured)("shopify smoke (live)", () => {
  it(
    ...
    if (products.length === 0) {
      console.info("[shopify smoke] catálogo vazio: importação do ERP ainda não ativada.");
      expect(requireCatalog, "Defina SHOPIFY_REQUIRE_LIVE_CATALOG=true depois da importação real.").toBe(false);
      return;
    }
